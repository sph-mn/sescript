(define-module (sph lang ecmascript expressions))

(use-modules (srfi srfi-1) (sph)
  (sph list) ((sph alist) #:select (alist alist-map))
  ( (sph string) #:select
    (any->string parenthesise parenthesised? string-enclose string-replace-string)))

(export es-apply es-array
  es-chain es-comma-join
  es-curly-brackets es-declare
  es-define es-escape-single-char
  es-for es-function
  es-get es-identifier
  es-if es-if-expression
  es-new es-object
  es-regexp es-set
  es-square-brackets es-string
  es-switch es-try-catch-finally es-value sph-lang-ecmascript-expressions-description)
